...
As the footer houses 12 neodymium magnets, considerations have to be made for 
the way in which these magnets interact with the surroundings of the footer as 
they could cause damage \ interfere with to mounted equipment or surrounding 
equipment i.e a hard disk located within the near vicinity of the footer. Using 
a software package called MagNet\textsuperscript{\textregistered} produced by 
infolytica\textsuperscript{\textregistered}, it was possible to 
perform a 2D finite element analysis of the footers, allowing the effects of 
these magnets to be modelled. When modelling the effect of these magnets the 
two extreme cases were considered: at minimum and maximum separation as 
detailed in Figure~\ref{figure:minsep} and Figure~\ref{figure:maxsep}.
...
Considering figures \ref{figure:minsep} and \ref{figure:maxsep} it can be seen 
that the flux density outside of the footer is negligible as it has a value of 
the order $10^-5~\si{\tesla}$ meaning that there would be minimal to no 
interactions between the footer and its surroundings as nearly all of the 
magnetic flux is contained within the core of the device.
...
the limits into Equation~\ref{?}, it can be determined the damping coefficient 
lies in the range $158.7182~\si{\newton\second\per\meter} < c < 
...
The design features 7 screws, 6 of which are to be used to attach the retaining 
wall to the core, with the other being used as a means of preloading the 
system. All of these screws are to be made of A2 tool Steel which has a yield 
stress, $\sigma_{y}$, of 205Mpa in compression and therefore through Von Mises 
yield criterion it is possible to achieve its yield stress in shear,
\begin{equation}
k = \frac{\sigma_{y}}{\sqrt{3}}
\end{equation}
where, $k$, is the yield stress of the A2 tool steel in shear and is equal to 
118Mpa.

Due to the nature of the retaining wall used to constrain the pre-loading 
backstops in their tracks, there is a shear force applied to the 6 retaining 
wall screws. At the worst possible instance the shearing force on one of these 
screws will be 1/6th of the total weight resting on the footer. It was decided 
that a single footer should not structurally fail in the event of an operator 
stepping on a footer by accident, meaning that these 6 retaining wall screws 
were specified for a 140kg mass. However, as these screws are to be tightly 
fastened to 2/3rds of the applied load per screw in order to increase the 
friction between the core and the screw and therefore increase the screws 
resistance to loosening. As 2/3rds of the load applied to the screw will come 
from the pre-loading / tightening, the screw must be not fail under a load of 3 
x the applied load per screw, so effectively 1/2 the overall applied load. From 
this an analysis was performed on the screws to find what diameter would be 
...
with a uniformly distributed load across its top edge as can be seen in figure 
\ref{diagram:screw}. Using this model the maximum shear stress within the screw 
was considered for various diameters of ISO and DIN metric threads, the results 
of this can be seen in table \ref{table:stresses retainer}. 

% Diagram of cantilever model 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Images/retainerscrew.png}
    \caption{How the retaining wall screws were modelled}
\end{figure}
\label{diagram:screw}

% Table of stress analysis for retainer screws 
\begin{table}[H]
    \centering
    \caption{Stress concentration analysis of the retainer screws}
    \begin{tabular}{llll}
        \hline
        Metric thread & D (mm) & $\tau$ (Mpa) & Will failure occur?\\
        \hline
        M1.6 & 1.25 & 586.8 & Yes\\
        M2 & 1.60 & 358.1 & Yes\\
        M2.5 & 2.05 & 218.1 & Yes\\
...
When considering the pre-loading screw at the bottom of the piece, the load is 
applied along its top edge, so it may be modelled as a rectangle with a single 
point load on top. Again, the worst possible case of applied load to the screw 
would be the full 140kg mass being supported by it as may be seen in figure 
\ref{table:stresses pre-load}. Therefore, the maximum stress case within the 
screw was considered for this applied load and tested for various diameters of 
ISO and DIN metric threads, as can be seen in table \ref{table:stresses 
pre-laod}. 

% Figure for pre-load screw
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Images/preloadscrew.png}
    \caption{How the pre-loading screw was modelled}
\end{figure}
\label{diagram:pre-load screw}

% Table of stress analysis for pre-loading screw 	
\begin{table}[H]
    \centering
    \caption{Stress concentration analysis of the retainer screws}
    \begin{tabular}{llll}
    ...
    \end{tabular}
\end{table}
\label{table:stresses pre-load}

As can be seen from Table~\ref{table:stresses retainer} and 
\ref{table:stresses pre-load} for both cases an M4 thread screw meets the 
...
induced into the pieces as the maximum mass which the design has been specified 
to work for is a 40kg mass supported by 3 footers (\textbf{ASK BENEDICT WHAT WE 
CAN PRE-LOAD UP TO}) meaning that the 6 retaining wall screws have a factor of 
safety of 151 and for the pre-loading screw the factor of safety of 131.
