...
end, namely between \textcolor{red}{£1500--£2000} per mount. 
...
(Fd(y)) is supplied by the vibration of the Hi-fi system and the opposing 
forces from the magnetic repulsion force (Fm(y)) and damping coefficient (c). 
Considering these components, it was possible to develop a second order 
differential equation (see equation \ref{eq:2ndDiff}) which when solved would 
...
In both graphs, the fundamental frequency can no longer be observed; a peak at around 30Hz is apparent however its low power and frequency that differs from 50Hz indicate that this is just noise. The noise is effectively identical in both cases and hence it can be concluded that the difference in damping coefficient between the 0.2 range of coefficient of friction is negligible. For further analysis, the median coefficient of friction will be used, 0.5; this returns a damping coefficient of 198.3978Ns/m.

The amp on data is displayed with the mathematically damped spectrum for comparison between the input and output of the dynamic model in Figure~\ref{fig:DampedVUndamped}.
...
The red curve shows the mathematically damped data; the fundamental frequency and its harmonics are indistinguishable from the noise. The entire signal has been reduced to below the noise floor of around -140dBV. The noise has been almost completely attenuated.
The success of the footers is likely to be less significant than that of the model. The primary reason for this discrepancy is the value of damping coefficient used; the damping factor was simply estimated and the damping coefficient calculated mathematically and assumed to be constant. Ideally, this would be found empirically; a series of prototypes would be produced and a similar experiment carried out to find the true damping coefficient. However, it has not been possible to do so and hence a mathematical estimation used instead. 
If the footers have a fraction of the successs they have been predicted to have, there will be a significant improvement in sound quality. 
...
Neodymium magnets are the most widely used type of rare-earth magnet, are the strongest type of permanent magnet commercially available and are of greatest benefit in applications where there is limited space. The magnetic properties termed remanence, coercivity and energy product dictate the performance of neodymium magnets. They measure the strength of the magnetic field, the material?s resistance to becoming demagnetized and the density of magnetic energy respectively. The notable mechanical properties are flexural strength, tensile strength, compressive strength and Vickers hardness.  In comparison with another popular type of rare-earth magnet, Samarium-Cobalt (SmCo), the properties mentioned are significantly higher. Given that there are numerous grades available, an overview of the range of values are outlined in Table 1. [1] The grade chosen for the project was N45SH, coated with three layers (nickel, copper and nickel) to reduce corrosion and provide a smooth finish. The brittleness of the magnets was also compensated for such that it does not undergo shear force during the cyclic loading. 

Magnetism is fundamentally a quantum property and only atoms with approximately half-filled outer shells can be magnetic because the electrons spin in the same direction. However, a material that is made up of these atoms is not necessarily magnetic. A domain is a collection of atoms whereby each of their magnets fields are aligned. To give rise to a permanent magnet, these domains must also align with one another.  In the pursuit of determining the lifetime of rare-earth permanent magnets, a number of factors must be considered which randomise the orientation of these domains:

1)	The Curie temperature.
2)	Alternating fields.
3)	Mechanical deformation.

It is thought that a modern SmCo magnet takes approximately 700 years to lose half its strength. [2] Even with the magnets placed in alternating fields, from Table 1, it is clear that the half-life of neodymium magnets is notably higher due to the difference in coercivity. The Curie temperature for neodymium magnets, the temperature at which the material loses all of its magnetic properties, is relatively low and was of great concern. Fortunately, the maximum temperature the grade can withstand (150C) was more than adequate for the project and therefore did not considerably affect the footer?s lifetime. [3]

TABLE 

\subsubsection{Tungsten Carbide Spherical Bearings}
Tungsten carbide possesses Young?s modulus 530 GPa, more than two times of that for the grade of stainless steel chosen (190 ? 203 GPa). [4] [5] As a result, the bearings do not require any lubrication and are able to ground any particulates present in the system, stopping them from interfering with the magnets and increasing the longevity of the design. The material does not undergo large deflection which in turn ensured efficient energy transfer between the Hi-Fi components.

\subsubsection{Stainless Steel and Aluminium Framework}
Stainless streel possesses a reasonable electrical conductivity (1.37  106 Siemens/m) and this prevented significant magnetic flux leakage. This improved the mount?s damping capability because the material essentially acted as a Faraday cage with high resistance. Stainless steel is totally recyclable and together with its high resistance to corrosion, greatly increased the lifetime and hence sustainability of the product. However, it was decided that the mount, apart from the top piece, core and spike, should instead be machined with aircraft grade aluminium (6061-T6) due to the steep manufacturing costs associated with stainless steel. These parts were excluded because they were subjected to the greatest stress. Not only is aluminium cheaper to manufacture but is extremely lightweight, a far better conductor (36.9  106 Siemens/m), widely available and recycled and can be easily extruded at low cost to produce the complex geometries present. [6]
...
\begin{figure}[h]
    \centering
    \subfloat[Assembly Jig 
    Base]{{\includegraphics[width=0.4\textwidth]{Images/jigbase} }}%
    \qquad
    \subfloat[Assembly Jig 
    Sleeve]{{\includegraphics[width=0.3\textwidth]{Images/jigsleeve} }}%
...
\begin{figure}[h]
    \centering
    \subfloat[Stillpoints Mechanism 
    Diagram]{{\includegraphics[width=0.4\textwidth]{Images/Stillpoints} }}%
    \qquad
    \subfloat[Group 25 Mechanism 
    Diagram]{{\includegraphics[width=0.4\textwidth]{Images/Group25} }}%
